    ...
    ">
      {tools.map((tool) => (
        <button
          ...
          className={`
            flex flex-col items-center justify-center gap-1.5 border-none cursor-pointer bg-transparent
            transition-all duration-200 text-[9px] font-bold tracking-wider uppercase p-0
            max-[640px]:text-[8px]
            ${active === tool.id
              ? 'text-[#c05520]'
        ...
        >
          <div className={`
            flex items-center justify-center rounded-full w-[44px] h-[44px] transition-all duration-200
            max-[640px]:w-[36px] max-[640px]:h-[36px]
            ${active === tool.id
              ? 'bg-[#fb6c11] text-white shadow-[0_4px_12px_rgba(251,108,17,0.4)]'
              : 'bg-transparent text-[#000000]'
            }
          `}>
            {tool.icon}
          </div>
          <span className={`${active === tool.id ? 'text-[#000000]' : 'text-[#000000]'} font-semibold`}>{tool.label}</span>
        </button>
      ))}
    </aside>
  );
